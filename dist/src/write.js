...
import { ATOM, COMPOUND, NUMBER, STRING, VAR, Env, deref, isCons, isEmptyList, } from './term.js';
...
function atomNeedsQuotes(name) {
    ...
    for (const ch of name)
        if (!graphicAtomCharacters.has(ch))
            return true;
    return false;
}
...
function operatorTable(definitions) {
    ...
    for (const definition of list) {
        if (!definition || definition.priority === 0)
            continue;
...
function format(term, env, options, table, maxPriority = 1200, context = 'term') {
    ...
    if (resolved.type === VAR)
        return options.variableNames.get(resolved.name) ?? writeVariable(resolved.name);
    if (resolved.type === STRING)
        return writeString(resolved.name);
    if (resolved.type === ATOM) {
        ...
        if (options.operatorAtomsAsArgs && context === 'argument' && table.has(resolved.name))
            return operatorName(resolved.name);
