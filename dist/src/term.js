...
export class Env {
    ...
    takeReadyDelays() {
        if (this._delays == null || this._delays.size === 0)
            return [];
        const ready = [];
        let remaining = this._delays;
        for (const [name, delays] of this._delays) {
            if (deref(variable(name), this).type === VAR)
                continue;
            if (remaining === this._delays)
                remaining = new Map(this._delays);
            remaining.delete(name);
            ready.push(...delays);
        }
        if (ready.length > 0)
            this._delays = remaining;
        return ready;
    }
}
    ...
    while (stack.length) {
        ...
        if (current?.type === VAR) {
            if (current.name === variableName)
                return true;
            if (seenVariables.has(current.name))
                continue;
        ...
        if (current?.type !== COMPOUND || seenTerms.has(current))
            continue;
    ...
    while (stack.length > 0) {
        ...
        if (a.type === COMPOUND) {
            if (a.name !== b.name || a.arity !== b.arity)
                return false;
            for (let i = a.arity - 1; i >= 0; i--)
                stack.push([a.args[i], b.args[i]]);
            continue;
        }
        return false;
    }
...
function writeVariable(name) {
    ...
    if (/^\?(?:[A-Za-z_][A-Za-z0-9_]*)?$/.test(name))
        return legacyVariableToIso(name);
    if (/^(?:_|[A-Z_][A-Za-z0-9_]*)$/.test(name))
        return name;
...
function quotedListText(term, env, doubleQuotes) {
    if (doubleQuotes !== 'chars' && doubleQuotes !== 'codes')
        return null;
    const characters = [];
    let cursor = term;
    while (true) {
...
function writeList(term, env, options) {
    ...
    if (quotedText != null)
        return writeString(quotedText, true);
    const parts = [];
    let cursor = term;
    while (true) {
        cursor = deref(cursor, env);
        ...
        if (!isCons(cursor)) {
            if (parts.length)
                return `[${parts.join(', ')} | ${termToString(cursor, env, true, options)}]`;
...
export function termToString(term, env = new Env(), quoteStrings = true, options = {}) {
    ...
    if (resolved.type === VAR)
        return writeVariable(resolved.name);
    if (isCons(resolved))
        return writeList(resolved, env, options);
    if (resolved.type === STRING)
        return writeString(resolved.name, quoteStrings);
    if (resolved.type === ATOM)
        return writeAtom(resolved.name);
    if (resolved.type === NUMBER)
        return resolved.name;
    if (resolved.type === COMPOUND && resolved.arity === 0)
        return writeAtom(resolved.name);
    ...
    return `${writeAtom(resolved.name)}(${args.map((arg) => termToString(arg, env, true, options)).join(', ')})`;
}
export function lexicalValue(term, env) {
    const resolved = deref(term, env);
    if (resolved.type === VAR)
        return null;
    if (resolved.type === ATOM || resolved.type === STRING || resolved.type === NUMBER)
        return resolved.name;
...
export function variantTerms(left, leftEnv, right, rightEnv, pairs = new Map(), reverse = new Map()) {
    ...
    for (let i = 0; i < left.arity; i++) {
        if (!variantTerms(left.args[i], leftEnv, right.args[i], rightEnv, pairs, reverse))
            return false;
    }
    return true;
}
export function compareTerms(left, right) {
    const rank = (term) => ({ [VAR]: 0, [NUMBER]: 1, [ATOM]: 2, [STRING]: 3, [COMPOUND]: 4 })[term.type] ?? -1;
    ...
    if (left.type === ATOM || left.type === STRING)
...
export function compareNumberText(left, right) {
    if (isDecimalInteger(left) && isDecimalInteger(right))
        return compareIntegerText(left, right);
