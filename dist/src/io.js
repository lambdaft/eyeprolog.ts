...
export class StreamManager {
    ...
    add(stream) {
        ...
        if (stream.alias)
            this.aliases.set(stream.alias, stream.id);
        return stream;
    }
    resolve(reference) {
    ...
    open(path, mode, options = {}) {
        ...
        if (!fs)
            throw new Error('file streams are unavailable in this runtime');
        ...
        if (mode === 'read')
            ...
            content = fs.readFileSync(path, type === 'binary' ? null : 'utf8');
        // @ts-expect-error TS7005: auto-suppressed
        if (BufferCtor?.isBuffer(content))
            content = [...content];
        return this.add({
    ...
    close(stream) {
        ...
        if (stream.alias)
            this.aliases.delete(stream.alias);
