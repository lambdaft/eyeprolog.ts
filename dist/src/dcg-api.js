...
export class DCG {
     ...
     */
    fromPrologTerm(term, env) {
        const t = deref(term, env);
        if (t.type === VAR)
            return `_var_${t.name}`;
        if (t.type === ATOM && t.name === '[]')
            return [];
        if (t.type === ATOM)
            return t.name;
        if (t.type === 'number')
        ...
        if (t.type === COMPOUND) {
            return { functor: t.name, args: t.args.map((a) => this.fromPrologTerm(a, env)) };
        }
        return String(t);
    }
    /**
     ...
     */
    parse(startRule, tokens) {
        if (!this.solver)
            throw new Error("No DCG rules loaded. Call load() or loadFile() first.");
     ...
     */
    parseWithBindings(startRuleSyntax, tokens) {
        if (!this.solver)
            throw new Error("No DCG rules loaded. Call load() or loadFile() first.");
        ...
        const extractVariables = (term) => {
            if (term.type === VAR)
                return [term.name];
            if (term.type === COMPOUND)
                return term.args.flatMap(extractVariables);
            return [];
        };
        ...
        for (const env of this.solver.solve([goal], new Env(), 0)) {
            ...
            for (const name of varNames) {
                if (name.startsWith('_') || name === 'Tokens')
                    continue; // skip internal or Tokens
     ...
     */
    generate(startRule, maxResults = 100) {
        if (!this.solver)
            throw new Error("No DCG rules loaded. Call load() or loadFile() first.");
