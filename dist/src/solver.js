...
import { COMPOUND, Env, compound, copyResolved, deref, emptyList, flattenConjunction, freshTerm, 
// @ts-expect-error TS6133: auto-suppressed
numberTerm, numberTextFromDouble, termIsGround, termToString, unify, variantTerms, variable, Term, } from './term.js';
...
function qualifyTerm(term, module) {
    if (!term || (term.type !== COMPOUND && term.type !== 'atom'))
        return term;
    term.module = module;
    for (const arg of term.args)
        qualifyTerm(arg, module);
    return term;
}
export function nextFreshId() {
    return ++freshCounter;
}
function raiseOccursCheckError(_left, _right, _env) {
...
export class Solver {
    constructor(program, options = {}) {
        ...
        if (this.isoStrict) {
            for (const name of [...this.prologFlags.keys()]) {
                if (!ISO_CORE_FLAG_NAMES.has(name))
                    this.prologFlags.delete(name);
            }
        }
            ...
            for (const [input, output] of program.charConversionDirectives ?? []) {
                if (input.type === 'atom' && output.type === 'atom' &&
                    ...
                    if (input.name === output.name)
                        this.charConversions.delete(input.name);
                    else
                        this.charConversions.set(input.name, output.name);
                }
            }
        }
    ...
    absorbStatsFrom(child) {
        if (!child || child === this || !child.stats)
            return;
    ...
    runInitializations() {
        for (const goal of this.program.initializations ?? []) {
            ...
            if (!succeeded)
                throw new PrologError('initialization_error');
        }
    }
    *solve(goals, env = new Env(), depth = 0) {
        if (depth === 0) {
            for (const result of this._solveInner(goals, env, depth)) {
                this.solutionsSeen++;
                yield result;
                if (this.solutionsSeen >= this.solutionLimit)
                    break;
            }
        }
        else {
            yield* this._solveInner(goals, env, depth);
        }
    }
    *_solveInner(goals, env = new Env(), depth = 0) {
        ...
        try {
            ...
            while (stack.length) {
                ...
                if (frame.kind === 'completeTableFixpointRound') {
                    // @ts-expect-error TS18048: auto-suppressed
                    if (frame.revision !== this.programRevision)
                        continue;
                    // @ts-expect-error TS18048: auto-suppressed
                ...
                while (true) {
                    ...
                    if (first?.kind === 'memoStore') {
                        if (first.revision === this.programRevision)
                    ...
                    if (goal.type === 'atom' && goal.name === '!' && goal.arity === 0) {
                        ...
                        this.cutEpoch++;
                        if (marker)
                            marker.cutEpoch = (marker.cutEpoch ?? 0) + 1;
                        for (const solveStack of this.solveStacks) {
                            for (let i = solveStack.length - 1; i >= 0; i--) {
                                if (marker == null || solveStack[i].active?.includes(marker))
                                    solveStack.splice(i, 1);
                            }
                        }
                        goals = rest;
                        depth++;
                    ...
                    if (goal.type === COMPOUND && goal.name === ':' && goal.arity === 2) {
                        ...
                        if (module.type === 'var')
                            throw new PrologError('instantiation_error');
                        if (module.type !== 'atom')
                            throw new PrologError('type_error(atom)', module);
                    ...
                    if (!callable)
                        throw new PrologError('type_error(callable)', goal);
                    ...
                    if (!group) {
                        if (goal.name === '-->' && goal.arity === 2) {
                            throw new PrologError('existence_error(procedure)', compound('/', [compound('-->', []), numberTerm(2)]));
                        }
                        if (this.prologFlags.get('unknown')?.value?.name === 'error') {
                            throw new PrologError('existence_error(procedure)', compound('/', [compound(goal.name, []), numberTerm(goal.arity)]));
                        }
                        break;
                    }
                    ...
                    if (group.tabled) {
                        ...
                        if (key.hasBound) {
                            ...
                            if (this.tableCoordinator)
                                this.tableCoordinator.entries.add(entry);
                            if (entry.complete) {
                    ...
                    if (!group.tabled && tryPushScalarFactRunFrames(stack, this, [goal, ...rest], env, depth, active))
                        break;
            ...
            if (stackIndex >= 0)
                this.solveStacks.splice(stackIndex, 1);
    ...
    *solveUserGoal(goal, rest, env, depth) {
        ...
        if (this.solutionsSeen >= this.solutionLimit && depth === 0)
            return;
    ...
    *solveMemoizedGoal(_group, goal, rest, env, depth) {
        yield* this._solveInner([goal, ...rest], env, depth);
    }
        ...
        for (const pass of [candidates.primary, candidates.fallback]) {
            for (let candidateIndex = 0; candidateIndex < clauseCandidateLength(pass); candidateIndex++) {
                ...
                if (clause.body.length === 0 && clause.scalarHead) {
                    ...
                    yield* this._solveInner(rest, next, depth + 1);
                    if (this.solutionsSeen >= this.solutionLimit && depth === 0)
                        return;
                    continue;
                }
                if (headCannotMatch(goal, clause.head, env))
                    continue;
    ...
    *solveRuleBodyThenRest(goal, goalEnv, body, rest, env, depth) {
     ...
     */
    static createEyeProlog(program) {
        return new Solver(program, {
            registry: getEyePrologRegistry(),
            maxDepth: 100000,
            solutionLimit: 10000000,
...
function scheduleTableFixpointRound(stack, solver, frame) {
    ...
    pushUserGoalUncachedFrames(stack, solver, frame.group, frame.goal, [{ kind: 'memoStore', entry: frame.entry, goal: frame.goal, revision: solver.programRevision }], frame.env, frame.depth, frame.active);
}
function pushMemoAnswerFrames(stack, entry, goal, rest, env, depth, active, solver) {
    for (let answerIndex = entry.answers.length - 1; answerIndex >= 0; answerIndex--) {
        ...
        for (let i = 0; i < goal.arity; i++) {
            ...
            if (!unify(goal.args[i], answerArgs[i], next)) {
                ok = false;
                break;
            }
        }
...
function pushUserGoalUncachedFrames(stack, solver, group, goal, rest, env, depth, active) {
    if (group.recursive && !group.cutRecursive && !group.linearNumeric && activeVariantIn(goal, env, active))
        return;
    if (solver.fastPathsEnabled && group.fastPi && pushFastPiFrames(stack, goal, rest, env, depth, active))
        return;
    if (tryPushGroundChainFrames(stack, solver, group, goal, rest, env, depth, active))
        return;
    const candidates = selectClauseCandidates(group, goal, env);
    const frames = [];
...
function pushFastPiFrames(stack, goal, rest, env, depth, active) {
    ...
    const next = env.clone();
    if (!unify(goal.args[3], numberTerm(numberTextFromDouble(sum)), next))
        return true;
    stack.push({ kind: 'goals', goals: rest, env: next, depth: depth + 1, active });
    return true;
}
...
function tryPushScalarFactRunFrames(stack, solver, goals, env, depth, active) {
    ...
    while (runLength < goals.length) {
        ...
        if (!goal || goal.kind === 'releaseActive' || goal.kind === 'memoStore')
            break;
        ...
        if (!group || group.tabled || !group.scalarFactsOnly)
            break;
        groups.push(group);
        runLength++;
    }
    ...
    while (localStack.length) {
        ...
        if (state.index === runLength) {
            ...
            for (let i = 0; i < state.names.length; i++)
                next.bind(state.names[i], state.values[i]);
            // @ts-expect-error TS18048: auto-suppressed
            frames.push({ kind: 'goals', goals: rest, env: next, depth: state.depth, active });
            if (frames.length > SCALAR_FACT_RUN_FRAME_LIMIT)
                return false;
            continue;
        }
        ...
        for (let i = nextStates.length - 1; i >= 0; i--)
            localStack.push(nextStates[i]);
        if (solver.solutionsSeen >= solver.solutionLimit)
            break;
    }
    for (let i = frames.length - 1; i >= 0; i--)
        stack.push(frames[i]);
...
function activeMightContain(goal, active) {
    if (active.length === 0 || goal.type !== COMPOUND)
        return false;
    for (const entry of active) {
        const activeGoal = entry.goal;
        if (activeGoal?.type === COMPOUND && activeGoal.name === goal.name && activeGoal.arity === goal.arity)
            return true;
    }
    return false;
}
...
function matchScalarFactLocal(goal, head, env, names, values) {
    if (goal.type !== COMPOUND || head.type !== COMPOUND)
        return null;
    if (goal.name !== head.name || goal.arity !== head.arity)
        return null;
    let nextNames = names;
...
function derefScalarMatch(term, env, names, values) {
    ...
    for (let guard = 0; current?.type === 'var' && guard < 128; guard++) {
        ...
        if (localIndex >= 0)
            current = values[localIndex];
        else if (env.has(current.name))
...
function tryPushGroundChainFrames(stack, solver, group, goal, rest, env, depth, active) {
    ...
    while (true) {
        ...
        for (const pass of [candidates]) {
            for (let candidateIndex = 0; candidateIndex < clauseCandidateLength(pass); candidateIndex++) {
                const clause = clauseCandidateAt(pass, candidateIndex);
                if (headCannotMatch(currentGoal, clause.head, currentEnv))
                    continue;
...
function matchGroundClause(goal, clause) {
    if (clause.head.type !== COMPOUND || goal.type !== COMPOUND)
        return undefined;
    if (clause.head.name !== goal.name || clause.head.arity !== goal.arity)
        return null;
    if (goal.arity === 2)
        return matchGroundBinaryClause(goal, clause);
    const names = [];
    const values = [];
    for (let i = 0; i < goal.arity; i++) {
        ...
        if (headArg.type === 'var') {
            ...
            if (!sameGroundTerm(headArg, goalArg))
                return null;
        }
        else {
            return undefined;
        ...
        if (headArg.type === 'var') {
            for (let j = 0; j < i; j++) {
                if (headArgs[j].type === 'var' && headArgs[j].name === headArg.name &&
                    !sameGroundTerm(goalArgs[j], goalArgs[i]))
                    return null;
            }
        }
        else if (isScalarTerm(headArg)) {
            if (!sameGroundTerm(headArg, goalArgs[i]))
                return null;
        }
        else {
            return undefined;
...
function sameGroundTerm(left, right) {
    ...
    if (arity !== (right.args?.length ?? 0))
        return false;
    for (let i = 0; i < arity; i++)
        if (!sameGroundTerm(left.args[i], right.args[i]))
            return false;
    return true;
}
function groundChainKey(term) {
    if (term?.type === COMPOUND) {
        ...
        for (let i = 0; i < term.arity; i++)
            out += `${groundChainKey(term.args[i])}`;
        return out;
    }
...
function rememberGroundChainSuccess(solver, seen) {
    ...
    for (const key of seen) {
        if ((index & 63) === 0 || index === last)
            solver.groundChainSuccess.add(key);
        index++;
    }
}
...
function activeVariantIn(goal, env, active) {
    ...
    for (const entry of active) {
        ...
        if (goalShape !== entry.variantShape)
            continue;
        if (variantTerms(goal, env, candidate, entry.env))
            return true;
    }
    return false;
}
...
function variantArgumentSize(term, env) {
    ...
    while (pending.length > 0) {
        ...
        if (current?.type === COMPOUND) {
            for (let index = 0; index < current.arity; index++)
                pending.push(current.args[index]);
        }
    }
    return size;
}
...
function selectReadyDeterministicBuiltin(goals, env, registry) {
    for (let i = 0; i < goals.length; i++) {
        const goal = goals[i];
        if (goal?.kind === 'releaseActive' || goal?.kind === 'memoStore')
            return 0;
        if (goal.type !== COMPOUND && goal.type !== 'atom')
            continue;
...
function headCannotMatch(goal, head, env) {
    ...
    for (let i = 0; i < goal.arity; i++) {
        ...
        if (isScalarTerm(da) && isScalarTerm(b) && !sameScalarTerm(da, b))
            return true;
    }
    return false;
}
...
function canonicalTermInfo(term, env, variables) {
    ...
    if (!value.args?.length)
        return { key: `${value.type}:${value.name}`, ground: true };
    let ground = true;
...
function copyResolvedWithKey(term, env, variables) {
    ...
    return {
        term: compound(value.name, children.map((child) => child.term)),
