...
import { ISO_OPERATOR_DEFINITIONS, QUAD_OPERATOR_DEFINITIONS, createParserOperatorState, parseClauses, parseClausesInto } from './parser.js';
...
export class Program {
    constructor(clauses = [], options = {}) {
        ...
        if (options[DEFER_PROGRAM_BUILD] === true)
            return;
    ...
    defineOperator(priority, specifier, name) {
        const operatorClass = ['fx', 'fy'].includes(specifier) ? ['fx', 'fy']
            : ['xf', 'yf'].includes(specifier) ? ['xf', 'yf']
                : ['xfx', 'xfy', 'yfx'];
        for (const existing of operatorClass)
            this.operators.delete(`${existing}\u0000${name}`);
        const key = `${specifier}\u0000${name}`;
        if (priority !== 0)
            this.operators.set(key, { priority, specifier, name });
    }
    ...
    _indexClause(clause, initialBuild) {
        ...
        if (!initialBuild)
            assertHeadIsDefinable(head, this.strictIso);
    ...
    importModule(target, source, requested = null) {
        ...
        if (!definition)
            throw new PrologError('existence_error(module)', atom(source));
    ...
    defineMetaPredicate(module, template) {
        ...
        if (group)
            group.metaArgumentPositions = positions;
    }
    ...
    removeDynamicClause(group, clause) {
        ...
        if (allIndex >= 0)
            this.clauses.splice(allIndex, 1);
        rebuildGroupIndexes(group);
        this.noteMutation(clause.body.length > 0);
        return true;
    }
    ...
    analyzeNegationStratification() {
        ...
        for (let component = 0; component < sccs.length; component++) {
            for (const index of sccs[component])
                componentByIndex.set(index, component);
        }
        const violations = [];
        const seen = new Set();
        for (const edge of edges) {
            ...
            if (from == null || to == null)
                continue;
            if (componentByIndex.get(from) !== componentByIndex.get(to))
                continue;
        ...
        for (const group of groups)
            group.negationStratum = strata.get(groupKeys.get(group)) ?? null;
    ...
    sourceFactLines(predicateKeys = null, options = {}) {
        ...
        for (const clause of this.clauses) {
            if (false) {
                if (clause.bodyName != null)
                    continue;
                if (predicateKeys && !predicateKeys.has(`${clause.headName}/2`))
                    continue;
                lines.add(`${formatTermForWrite(clause.head, env, writeOptions)}.\n`);
                continue;
            }
            if (clause.body.length !== 0 || (clause.head.type !== ATOM && clause.head.type !== COMPOUND))
                continue;
            if (predicateKeys && !predicateKeys.has(`${clause.head.name}/${clause.head.arity}`))
                continue;
            lines.add(`${formatTermForWrite(clause.head, env, writeOptions)}.\n`);
        }
        return lines;
    ...
    initializations;
    quads;
    prologFlagDirectives;
    charConversionDirectives;
    doubleQuotes;
}
class ProgramBuilder {
    ...
    addClauses(clauses) {
        if (this.finished)
            throw new Error('program builder is already finalized');
        ...
        for (const clause of clauses) {
            ...
            if (false) {
                ...
                if (clause.bodyName != null || !clause.scalarHead)
                    group.scalarFactsOnly = false;
    ...
    addDirectiveClause(clause) {
        ...
        if (operator) {
            for (const name of operator.names)
                program.defineOperator(operator.priority, operator.specifier, name);
        }
    ...
    finish() {
        ...
        if (this.options.strictNegation === true)
...
function buildProgramFromSources(sources, options) {
    ...
    if (options.sourceMetadata === false && !hasModuleDirectives) {
        ...
        if (loadSourcesIntoBuilder(builder, sources, options, true))
...
function loadSourcesIntoBuilder(builder, sources, options, fast) {
    ...
    try {
        for (const item of prepared) {
            ...
            if (!loadSourceIntoBuilder(builder, text, item.options, ensured, loadedModules, fast, context, loadingModules))
                return false;
        }
...
function sourcePath(options) {
    ...
    if (!filename || filename.startsWith('<') || /^https?:\/\//.test(filename))
        return null;
    const base = options.baseDir ?? currentWorkingDirectory();
    return path.resolve(base, filename);
}
function loadSourceIntoBuilder(builder, source, options, ensured, loadedModules, fast, context, loadingModules = new Set()) {
    // @ts-expect-error TS7034: auto-suppressed
    const batch = [];
    const flush = () => {
        if (batch.length === 0)
    ...
    const accept = (clause) => {
        ...
        if (!include) {
            ...
            if (!isDirectiveClause(clause)) {
                for (const goal of clause.body)
                    annotateGoalModule(goal, clause.module);
            }
            batch.push(clause);
            if (batch.length >= PROGRAM_BUILD_BATCH_SIZE)
                flush();
            return;
        }
        flush();
...
function moduleDirective(clause) {
    ...
    if (directive?.type !== COMPOUND || directive.name !== 'module' || directive.arity !== 2)
        return null;
    if (directive.args[0].type !== ATOM)
        throw new PrologError('type_error(atom)', directive.args[0]);
...
function useModuleDirective(clause) {
    ...
    if (directive.arity === 2 && imports == null)
        throw new PrologError('type_error(list)', directive.args[1]);
...
function readModuleSource(designation, options) {
    ...
    if (designation.type !== ATOM)
        throw new PrologError('type_error(source_sink)', designation);
    // @ts-expect-error TS7005: auto-suppressed
    if (!fs || !path)
        throw new PrologError('permission_error(access, source_sink)', designation);
    const base = options.baseDir ?? currentWorkingDirectory();
    ...
    if (!declaration)
        throw new PrologError('existence_error(module)', designation);
...
function annotateGoalModule(term, module) {
    if (!term || (term.type !== ATOM && term.type !== COMPOUND))
        return term;
    term.module = module;
    const callableArguments = (term.name === ',' || term.name === ';' || term.name === '->') ? term.args
        : (['call', 'once', '\\+', 'not', 'catch', 'forall', 'findall', 'bagof', 'setof',
            ...
            ? term.args
            : []);
    for (const arg of callableArguments)
        annotateGoalModule(arg, module);
    return term;
}
function readIncludedSource(directive, options, ensured) {
    const designation = directive.args[0];
    if (designation.type !== ATOM)
        throw new PrologError('type_error(atom)', designation);
    // @ts-expect-error TS7005: auto-suppressed
    if (!fs || !path) {
        throw new PrologError('permission_error(access, source_sink)', atom(designation.name));
    }
    const base = options.baseDir ?? (options.filename && path.isAbsolute(String(options.filename))
        ? path.dirname(path.resolve(options.filename))
        : currentWorkingDirectory());
    const filename = path.resolve(base, designation.name);
    if (directive.name === 'ensure_loaded' && ensured.has(filename))
        return null;
    if (directive.name === 'ensure_loaded')
        ensured.add(filename);
    let text;
    try {
        text = fs.readFileSync(filename, 'utf8');
...
function dynamicDirectiveIndicators(clause) {
    if (!isDirectiveClause(clause))
        return [];
    const directive = clause.head.args[0];
    if (directive.type !== COMPOUND || directive.name !== 'dynamic' || directive.arity !== 1)
        return [];
    const terms = properListItems(directive.args[0], new Env()) ?? flattenDirectiveSequence(directive.args[0]);
    return terms.map((indicator) => indicator.type === COMPOUND && ['/', '//'].includes(indicator.name) && indicator.arity === 2
...
function operatorDirective(clause) {
    ...
    if (priority.type !== 'number' || specifier.type !== ATOM)
        return null;
    const items = names.type === ATOM ? [names] : properListItems(names, new Env());
    if (!items || items.some((item) => item.type !== ATOM))
        return null;
    return {
...
function staticProcedureModificationError(name, arity) {
    return new PrologError('permission_error(modify, static_procedure)', compound('/', [atom(name), numberTerm(arity)]));
}
...
function clauseIsDirectRecursive(clause, group) {
    if (false)
        return false;
    return clause.body.some((goal) => goal.type === COMPOUND && goal.name === group.name && goal.arity === group.arity);
}
function componentHasCut(start, deps, groups) {
    ...
    return component.some((index) => {
        ...
        if (!directRecursive)
            return group.clauses.some(clauseHasCut);
    ...
    for (const clause of group.clauses) {
        ...
        for (const goal of clause.body) {
            ...
            for (let index = 0; index < group.arity; index++) {
                if (!sameClauseTerm(clause.head.args[index], goal.args[index]))
                    changed[index] = 1;
            }
        }
        if (!recursive)
        ...
        for (let index = 0; index < group.arity; index++) {
            ...
            if (headArg.type !== 'var' && (firstPatternedPosition < 0 || index < firstPatternedPosition))
                firstPatternedPosition = index;
            if (headArg.type !== 'var' || changed[index] === 0)
                continue;
            for (let patternIndex = 0; patternIndex < group.arity; patternIndex++) {
                ...
                if (pattern.type !== 'var' && termContainsVariable(pattern, headArg.name)) {
                    if (firstLinkedInputPosition < 0 || index < firstLinkedInputPosition)
                        firstLinkedInputPosition = index;
                    break;
                }
            }
        }
    }
    if (firstLinkedInputPosition >= 0)
        return [[firstLinkedInputPosition]];
    if (firstPatternedPosition >= 0)
        return [[firstPatternedPosition]];
...
function hasLinearNumericRecursion(group) {
    ...
    return recursiveClause != null && recursiveClause.body.some((goal) => goal.type === COMPOUND && goal.name === 'is' && goal.arity === 2);
}
function isPiAccumulator(group) {
    return group.name === 'pi' && group.arity === 5 && group.clauses.some((clause) => clause.body.some((goal) => goal.type === COMPOUND && goal.name === 'is' && goal.arity === 2));
}
...
function sameClauseTerm(left, right) {
    if (left.type !== right.type || left.name !== right.name || left.args.length !== right.args.length)
        return false;
    return left.args.every((arg, index) => sameClauseTerm(arg, right.args[index]));
}
function termHasNoVariables(term) {
    if (!term || term.type === 'var')
        return false;
...
function collectGoalDependencies(goal, negated) {
    if (goal.type === ATOM)
        return [{ key: `${goal.name}/0`, name: goal.name, arity: 0, module: goal.module, negative: negated }];
    if (goal.type !== COMPOUND)
        return [];
    if (goal.name === ',' && goal.arity === 2) {
        return [
...
function computeNegationStrata(groups, edges, indexByKey) {
    ...
    for (let pass = 0; pass < groups.length; pass++) {
        let changed = false;
        for (const edge of edges) {
            if (!indexByKey.has(edge.from) || !indexByKey.has(edge.to))
                continue;
...
function addClauseBucket(buckets, key, clause) {
    ...
    if (existing == null)
        buckets.set(key, clause);
    else if (Array.isArray(existing))
        existing.push(clause);
    else
        buckets.set(key, [existing, clause]);
}
...
function indexCompactOne(index, type, name, clause, clauses = null, clausePosition = -1) {
    if (type !== VAR) {
        if (!index.sawScalar) {
            ...
            if (clauses && clausePosition > 0)
                index.fallback = clauses.slice(0, clausePosition);
        }
...
function rebuildGroupIndexes(group) {
    ...
    for (let clausePosition = 0; clausePosition < group.clauses.length; clausePosition++) {
        ...
        for (let i = 0; i < group.arity; i++)
            indexOne(group.argIndexes[i], clause.head.args[i], clause, group.clauses, clausePosition);
    }
}
...
function demandValueKey(values) {
    ...
    if (values.length === 1)
        return scalarIndexKey(values[0]);
...
function buildDemandIndex(group, positions) {
    ...
    for (const clause of group.clauses) {
        if (false) {
            const values = positions.map((position) => compactTerm(compactHeadArgType(clause, position), compactHeadArgName(clause, position)));
...
function mergeClausesInSourceOrder(primary, fallback) {
    ...
    while (left < primaryLength)
        merged.push(clauseCollectionAt(primary, left++));
    while (right < fallback.length)
        merged.push(fallback[right++]);
    return merged;
}
export function selectGroundClauseCandidates(group, goal) {
    ...
    for (let i = 0; i < goal.arity; i++) {
        ...
        if (fallback.length / group.clauses.length > INDEX_MAX_VAR_FRACTION)
            continue;
        const primary = argumentBucket(index, value);
        const length = clauseCollectionLength(primary) + fallback.length;
        if (group.clauses.length / Math.max(1, length) < INDEX_MIN_SPEEDUP)
            continue;
    ...
    if (bestFallback.length === 0)
        return bestPrimary;
    if (clauseCollectionLength(bestPrimary) === 0)
        return bestFallback;
