...
import { ATOM, COMPOUND, NUMBER, STRING, VAR, Env, atom, compareTerms, compound, copyResolved, deref, emptyList, isDecimalInteger, listFromItems, numberTerm, numberTextFromDouble, properListItems, termIsGround, termToString, unify, variable, variantTerms, } from './term.js';
...
let isoFresh = 0;
function formatErrorDetail(term) {
    if (term == null)
        return '';
...
export const isoBuiltins = {
    register(registry) {
        ...
        for (let arity = 2; arity <= 8; arity++)
            registry.add('call', arity, callClosureBuiltin);
...
function* unification({ goal, env }) {
    ...
    if (unify(goal.args[0], goal.args[1], next))
        yield next;
}
function* unificationWithOccursCheck({ goal, env }) {
    ...
    // an exception.
    if (unify(goal.args[0], goal.args[1], next, { occursCheck: 'fail' }))
        yield next;
}
function* nonUnification({ goal, env }) {
    if (!unify(goal.args[0], goal.args[1], env.clone()))
        yield env;
}
function termVariableNames(term, env, names = new Set(), seen = new Set()) {
    ...
    if (term.type === VAR) {
        ...
        for (const arg of term.args)
            termVariableNames(arg, env, names, seen);
    }
    return names;
}
function subsumesTerm(general, specific, env) {
    ...
    while (pending.length) {
        ...
        if (left.type === VAR && substitutions.has(left.name))
            left = substitutions.get(left.name);
        if (left.type === VAR) {
            ...
            if (protectedVariables.has(left.name)) {
                if (right.type !== VAR || right.name !== left.name)
                    return false;
            }
            else {
        ...
        if (left.type !== right.type || left.name !== right.name || left.arity !== right.arity)
            return false;
        for (let i = left.arity - 1; i >= 0; i--)
            pending.push([left.args[i], right.args[i]]);
    }
    return true;
}
function* subsumesTermBuiltin({ goal, env }) {
    if (subsumesTerm(goal.args[0], goal.args[1], env))
        yield env;
}
function* identity({ goal, env }) {
    if (identical(goal.args[0], goal.args[1], env))
        yield env;
}
...
function identical(left, right, env) {
    ...
    for (let i = 0; i < left.arity; i++)
        if (!identical(left.args[i], right.args[i], env))
            return false;
    return true;
}
const unaryTest = (predicate) => function* ({ goal, env }) {
    if (predicate(deref(goal.args[0], env), env))
        yield env;
};
const typeTests = {
...
function termIsAcyclic(term, env) {
    ...
    while (stack.length) {
        ...
        if (active.has(resolved))
            return false;
        if (complete.has(resolved))
            continue;
        active.add(resolved);
        stack.push([resolved, true]);
        for (let i = resolved.arity - 1; i >= 0; i--)
            stack.push([resolved.args[i], false]);
    }
    return true;
}
...
function* compareBuiltin({ goal, env }) {
    ...
    if (order.type !== VAR) {
        if (order.type !== ATOM)
            throw new PrologError('type_error(atom)', order);
        if (!['<', '=', '>'].includes(order.name))
            throw new PrologError('domain_error(order)', order);
    }
    const cmp = resolvedOrder(goal.args[1], goal.args[2], env);
    const next = env.clone();
    if (unify(goal.args[0], atom(cmp < 0 ? '<' : cmp > 0 ? '>' : '='), next))
        yield next;
}
function orderBuiltin(test) {
    return function* ({ goal, env }) {
        if (test(resolvedOrder(goal.args[0], goal.args[1], env)))
            yield env;
    };
}
...
function* sortBuiltin({ goal, env }) {
    ...
    for (const item of sorted) {
        if (unique.length === 0 || resolvedOrder(unique[unique.length - 1], item, env) !== 0)
            unique.push(item);
    }
    const next = env.clone();
    if (unify(goal.args[1], listFromItems(unique), next))
        yield next;
}
function* keysortBuiltin({ goal, env }) {
    ...
    const next = env.clone();
    if (unify(goal.args[1], listFromItems(sorted), next))
        yield next;
}
...
function requireAtom(term, env) {
    ...
    if (value.type === VAR)
        throw new PrologError('instantiation_error');
    if (value.type !== ATOM)
        throw new PrologError('type_error(atom)', value);
    return value;
}
function* functorBuiltin({ goal, env }) {
    ...
    if (term.type !== VAR) {
        ...
        if (unify(goal.args[1], name, next) && unify(goal.args[2], numberTerm(term.arity), next))
            yield next;
        return;
    }
    ...
    if (arity > BigInt(Number.MAX_SAFE_INTEGER))
        throw new PrologError('representation_error(max_arity)');
    if (arity === 0n) {
        if (name.type === VAR)
            throw new PrologError('instantiation_error');
        if (name.type === COMPOUND)
            throw new PrologError('type_error(atomic)', name);
        if (unify(goal.args[0], name, next))
            yield next;
        return;
    }
    if (name.type === VAR)
        throw new PrologError('instantiation_error');
    if (name.type !== ATOM)
        throw new PrologError('type_error(atom)', name);
    const id = ++isoFresh;
    if (unify(goal.args[0], compound(name.name, Array.from({ length: Number(arity) }, (_, i) => variable(`__functor${id}_${i}`))), next))
        yield next;
}
function* argBuiltin({ goal, env }) {
    ...
    if (term.type !== COMPOUND)
        throw new PrologError('type_error(compound)', term);
    if (index < 0n)
        throw new PrologError('domain_error(not_less_than_zero)', deref(goal.args[0], env));
    if (index === 0n || index > BigInt(term.arity))
        return;
    const next = env.clone();
    if (unify(goal.args[2], term.args[Number(index) - 1], next))
        yield next;
}
function* univBuiltin({ goal, env }) {
    ...
    if (term.type !== VAR) {
        ...
        if (unify(goal.args[1], listFromItems(items), next))
            yield next;
        return;
    }
    const items = properListItems(goal.args[1], env);
    if (items == null) {
        if (isPartialList(goal.args[1], env))
            throw new PrologError('instantiation_error');
        throw new PrologError('type_error(list)', deref(goal.args[1], env));
    }
    if (items.length === 0)
        throw new PrologError('domain_error(non_empty_list)', emptyList());
    if (items.length === 1) {
        const scalar = deref(items[0], env);
        if (scalar.type === VAR)
            throw new PrologError('instantiation_error');
        if (scalar.type === COMPOUND)
            throw new PrologError('type_error(atomic)', scalar);
        if (unify(goal.args[0], scalar, next))
            yield next;
        return;
    }
    const name = requireAtom(items[0], env);
    if (unify(goal.args[0], compound(name.name, items.slice(1)), next))
        yield next;
}
...
function freshCopy(term, env, variables = new Map(), id = ++isoFresh) {
    term = deref(term, env);
    if (term.type === VAR) {
        if (!variables.has(term.name))
            variables.set(term.name, variable(`__copy${id}_${variables.size}`));
...
function* copyTermBuiltin({ goal, env }) {
    const next = env.clone();
    if (unify(goal.args[1], freshCopy(goal.args[0], env), next))
        yield next;
}
function* termVariablesBuiltin({ goal, env }) {
    ...
    if (unify(goal.args[1], listFromItems(found), next))
        yield next;
}
...
function callableOrVariable(term, env) {
    const value = deref(term, env);
    if (value.type === VAR || value.type === ATOM || value.type === COMPOUND)
        return value;
...
function* clauseBuiltin({ solver, goal, env }) {
    ...
    if (head.type !== ATOM && head.type !== COMPOUND)
        throw new PrologError('type_error(callable)', head);
...
function* retractBuiltin({ solver, goal, env }) {
    ...
    for (const clause of candidates) {
        ...
        const next = env.clone();
        if (!unify(parts.head, copied.args[0], next))
            continue;
        if (parts.rule && !unify(parts.body, copied.args[1], next))
            continue;
        if (!parts.rule && !(copied.args[1].type === ATOM && copied.args[1].name === 'true'))
            continue;
...
function predicateIndicatorParts(term, env) {
    ...
    if (name.type === VAR || arity.type === VAR)
        throw new PrologError('instantiation_error');
    if (arity.type !== NUMBER || !isDecimalInteger(arity.name))
        throw new PrologError('type_error(integer)', arity);
    if (name.type !== ATOM)
        throw new PrologError('type_error(atom)', name);
    const integer = BigInt(arity.name);
    if (integer < 0n)
        throw new PrologError('domain_error(not_less_than_zero)', arity);
...
function* currentPrologFlagBuiltin({ solver, goal, env }) {
    ...
    if (flag.type !== VAR && flag.type !== ATOM)
        throw new PrologError('type_error(atom)', flag);
...
function* setPrologFlagBuiltin({ solver, goal, env }) {
    ...
    if (flag.type === VAR || value.type === VAR)
        throw new PrologError('instantiation_error');
    if (flag.type !== ATOM)
        throw new PrologError('type_error(atom)', flag);
    ...
    if (!definition.changeable)
        throw new PrologError('permission_error(modify, flag)', flag);
...
function operatorPriority(term, env) {
    ...
    if (priority < 0n || priority > 1200n)
        throw new PrologError('domain_error(operator_priority)', value);
...
function operatorSpecifier(term, env) {
    ...
    if (value.type !== ATOM)
        throw new PrologError('type_error(atom)', value);
    if (!operatorSpecifiers.has(value.name))
        throw new PrologError('domain_error(operator_specifier)', value);
    return value.name;
}
function operatorNames(term, env) {
    ...
    for (const item of items) {
        ...
        if (resolved.type !== ATOM)
            throw new PrologError('type_error(atom)', resolved);
    }
...
function* opBuiltin({ solver, goal, env }) {
    ...
    for (const name of operatorNames(goal.args[2], env)) {
        ...
        if (priority !== 0 && [...solver.program.operators.values()].some((definition) => definition.name === name.name &&
...
function conversionCharacter(term, env, current = false) {
    ...
    if (!oneChar(value)) {
        if (current)
            throw new PrologError('type_error(character)', value);
...
function* charConversionBuiltin({ solver, goal, env }) {
    ...
    if (input.name === output.name)
        solver.charConversions.delete(input.name);
    else
        solver.charConversions.set(input.name, output.name);
    yield env;
}
...
function requireStream(solver, term, env, mode = null) {
    ...
    if (!stream)
        throw new PrologError('existence_error(stream)', culprit);
...
function optionAtom(option, name) {
    if (option.type !== COMPOUND || option.name !== name || option.arity !== 1)
        return null;
...
function openOptions(term, env) {
    ...
    for (const option of optionList(term, env)) {
        ...
        if ((value = optionAtom(option, 'type'))) {
            ...
            if (value.type !== ATOM)
                throw new PrologError('type_error(atom)', value);
            if (!['text', 'binary'].includes(value.name))
...
function* openBuiltin({ solver, goal, env }) {
    ...
    if (!['read', 'write', 'append'].includes(mode.name))
        throw new PrologError('domain_error(io_mode)', mode);
    const streamTarget = deref(goal.args[2], env);
    if (streamTarget.type !== VAR)
        throw new PrologError('uninstantiation_error', streamTarget);
    ...
    try {
        const next = env.clone();
        if (unify(goal.args[2], streamHandle(stream.id), next))
            yield next;
        else
            throw new Error('stream_unify_failed');
    }
    catch (error) {
        solver.io.close(stream);
        if (error instanceof PrologError || error instanceof ThrownTerm)
            throw error;
...
function* closeBuiltin({ solver, goal, env }) {
    ...
    if (!stream.standard) {
        if (solver.io.currentInput === stream.id)
            solver.io.currentInput = 0;
        if (solver.io.currentOutput === stream.id)
            solver.io.currentOutput = 1;
        solver.io.close(stream);
    }
    yield env;
}
function* currentInputBuiltin({ solver, goal, env }) {
    ...
    if (value.type !== VAR) {
        ...
        if (stream.id === solver.io.currentInput)
            yield env;
        return;
    }
    const next = env.clone();
    if (unify(goal.args[0], streamHandle(solver.io.currentInput), next))
        yield next;
}
function* currentOutputBuiltin({ solver, goal, env }) {
    ...
    if (value.type !== VAR) {
        ...
        if (!stream)
            throw new PrologError('domain_error(stream)', value);
        if (stream.id === solver.io.currentOutput)
            yield env;
        return;
    }
    const next = env.clone();
    if (unify(goal.args[0], streamHandle(solver.io.currentOutput), next))
        yield next;
}
function setCurrentStreamBuiltin(mode) {
    return function* ({ solver, goal, env }) {
        ...
        if (mode === 'read')
            solver.io.currentInput = stream.id;
        else
            solver.io.currentOutput = stream.id;
        yield env;
    };
}
...
function streamProperties(stream) {
    ...
    if (stream.alias)
        properties.push(compound('alias', [atom(stream.alias)]));
    if (stream.path)
        properties.push(compound('file_name', [atom(stream.path)]));
    return properties;
}
function* setStreamPositionBuiltin({ solver, goal, env }) {
    const stream = requireStream(solver, goal.args[0], env);
    if (!stream.reposition)
        throw new PrologError('permission_error(reposition, stream)', deref(goal.args[0], env));
    ...
    if (position.type === VAR)
        throw new PrologError('instantiation_error');
    if (position.type !== NUMBER || !isDecimalInteger(position.name))
        throw new PrologError('domain_error(stream_position)', position);
    const offset = BigInt(position.name);
    if (offset < 0n || offset > BigInt(stream.content.length))
        throw new PrologError('domain_error(stream_position)', position);
...
function inputUnitBuiltin(name) {
    return function* ({ solver, goal, env }) {
        ...
        if (binary !== (stream.type === 'binary'))
            throw new PrologError('permission_error(input, stream)', streamHandle(stream.id));
...
function outputUnitBuiltin(name) {
    return function* ({ solver, goal, env }) {
        ...
        if (name === 'put_char') {
            if (stream.type !== 'text')
                throw new PrologError('permission_error(output, binary_stream)', streamHandle(stream.id));
            if (!oneChar(value))
                throw new PrologError('type_error(character)', value);
            ...
            if (code < 0n || code > max)
                throw new PrologError(name === 'put_byte' ? 'type_error(byte)' : 'representation_error(character_code)');
...
function readTermFromStream(stream, solver) {
    ...
    for (const candidate of termTextCandidates(stream)) {
        ...
        try {
            ...
            if (clauses.length !== 1 || clauses[0].body.length)
...
function* readBuiltin({ solver, goal, env }) {
    ...
    const next = env.clone();
    if (unify(goal.args[goal.arity - 1], readTermFromStream(stream, solver), next))
        yield next;
}
function* readTermBuiltin({ solver, goal, env }) {
    ...
    const visit = (item) => {
        if (item.type === VAR) {
            ...
            // @ts-expect-error TS7005: auto-suppressed
            if (!variables.some((entry) => entry.name === item.name))
                variables.push(item);
        }
        else
    ...
    for (const option of options) {
        ...
        if (!unify(option.args[0], value, next))
            return;
    }
    yield next;
}
function defaultTermWriteOptions(mode) {
    if (mode === 'writeq')
        return { quoted: true, ignoreOps: false, numbervars: true, variableNames: new Map(), compact: true, operatorAtomsAsArgs: true };
    if (mode === 'canonical')
        return { quoted: true, ignoreOps: true, numbervars: false, variableNames: new Map(), compact: true, operatorAtomsAsArgs: true };
...
function writeVariableNames(value, env, option) {
    ...
    for (const item of items) {
        ...
        if (target.type === VAR && !names.has(target.name))
...
function termWriteOptions(term, env, mode = 'write') {
    ...
    for (const option of optionList(term, env)) {
        ...
        if (option.name === 'quoted')
            result.quoted = writeOptionBoolean(option.args[0], env, option);
        else if (option.name === 'ignore_ops')
            result.ignoreOps = writeOptionBoolean(option.args[0], env, option);
        else if (option.name === 'numbervars')
            result.numbervars = writeOptionBoolean(option.args[0], env, option);
        else if (option.name === 'variable_names')
            result.variableNames = writeVariableNames(option.args[0], env, option);
        else
            throw new PrologError('domain_error(write_option)', option);
    }
    return result;
...
function* atomLengthBuiltin({ goal, env }) {
    ...
    const next = env.clone();
    if (unify(goal.args[1], numberTerm(characters(value.name).length), next))
        yield next;
}
function* atomConcatBuiltin({ goal, env }) {
    ...
    if (first.type === VAR && whole.type === VAR)
        throw new PrologError('instantiation_error');
    if (second.type === VAR && whole.type === VAR)
        throw new PrologError('instantiation_error');
    const candidates = [];
    if (whole.type === ATOM && first.type === VAR && second.type === VAR) {
        const chars = characters(whole.name);
        for (let i = 0; i <= chars.length; i++)
            candidates.push([chars.slice(0, i).join(''), chars.slice(i).join(''), whole.name]);
    }
...
function optionalNonNegativeInteger(term, env) {
    ...
    if (integer < 0n)
        throw new PrologError('domain_error(not_less_than_zero)', value);
    return integer;
}
function* subAtomBuiltin({ goal, env }) {
    ...
    if (source.type === VAR)
        throw new PrologError('instantiation_error');
    if (source.type !== ATOM)
        throw new PrologError('type_error(atom)', source);
    ...
    for (let start = 0; start <= chars.length; start++) {
        if (before != null && before !== BigInt(start))
            continue;
        for (let size = 0; size <= chars.length - start; size++) {
            const remaining = chars.length - start - size;
            if (length != null && length !== BigInt(size))
                continue;
            if (after != null && after !== BigInt(remaining))
                continue;
            const text = chars.slice(start, start + size).join('');
            if (sub.type === ATOM && sub.name !== text)
                continue;
            const next = env.clone();
...
function listToAtomInput(list, env, kind) {
    ...
    if (tail.type === VAR || items.some((item) => item.type === VAR))
        throw new PrologError('instantiation_error');
    if (tail.type !== ATOM || tail.name !== '[]')
        throw new PrologError('type_error(list)', tail);
    if (kind === 'chars') {
        const invalid = items.find((item) => !oneChar(item));
        if (invalid)
            throw new PrologError('type_error(character)', invalid);
        return items.map((item) => item.name).join('');
    }
    const nonInteger = items.find((item) => item.type !== NUMBER || !isDecimalInteger(item.name));
    if (nonInteger)
        throw new PrologError('type_error(integer)', nonInteger);
...
function atomListBuiltin(kind) {
    return function* ({ goal, env }) {
        const value = deref(goal.args[0], env);
        if (value.type !== VAR && value.type !== ATOM)
            throw new PrologError('type_error(atom)', value);
        const list = deref(goal.args[1], env);
        if (value.type === VAR && list.type === VAR)
            throw new PrologError('instantiation_error');
        const next = env.clone();
        if (value.type === ATOM) {
            ...
            const items = characters(value.name).map((ch) => kind === 'chars' ? atom(ch) : numberTerm(ch.codePointAt(0)));
            if (unify(goal.args[1], listFromItems(items), next))
                yield next;
            return;
        }
        if (unify(goal.args[0], atom(listToAtomInput(list, env, kind)), next))
            yield next;
    };
}
...
function* charCodeBuiltin({ goal, env }) {
    ...
    if (char.type === VAR && code.type === VAR)
        throw new PrologError('instantiation_error');
    if (char.type !== VAR && !oneChar(char))
    ...
    if (code.type !== VAR && !validCharacterCode(code))
        throw new PrologError('representation_error(character_code)');
    const next = env.clone();
    if (char.type === ATOM) {
        if (unify(goal.args[1], numberTerm(char.name.codePointAt(0)), next))
            yield next;
    }
    else if (unify(goal.args[0], atom(String.fromCodePoint(Number(code.name))), next))
        yield next;
}
...
function parseIsoNumber(text) {
    ...
    if (/^-?\d+[eE][+-]?\d+$/.test(numericText))
        return null;
    try {
        const parsed = parseGoalText(`number_chars_value(${numericText})`);
        if (parsed.type !== COMPOUND || parsed.name !== 'number_chars_value' ||
            parsed.arity !== 1 || parsed.args[0].type !== NUMBER)
            return null;
        const value = parsed.args[0];
        if (isDecimalInteger(value.name))
            return numberTerm(BigInt(value.name).toString());
...
function numberListText(list, env, kind, valueIsBound) {
    ...
    if (tail.type !== VAR && !proper)
        throw new PrologError('type_error(list)', whole);
...
function numberListBuiltin(kind) {
    return function* ({ goal, env }) {
        const value = deref(goal.args[0], env);
        if (value.type !== VAR && value.type !== NUMBER)
            throw new PrologError('type_error(number)', value);
        const list = deref(goal.args[1], env);
        if (value.type === VAR && list.type === VAR)
            throw new PrologError('instantiation_error');
        ...
        if (value.type === NUMBER) {
            if (text != null) {
                ...
                if (parsed == null)
                    throw new PrologError('syntax_error(number)');
        ...
        if (parsed == null)
            throw new PrologError('syntax_error(number)');
        if (unify(goal.args[0], parsed, next))
            yield next;
    };
}
...
function* findallBuiltin({ solver, goal, env }) {
    ...
    const next = env.clone();
    if (unify(bag, listFromItems(collected), next))
        yield next;
}
function collectVariableNames(term, env, names = new Set()) {
    ...
    if (term.type === VAR) {
        ...
        for (const arg of term.args)
            collectVariableNames(arg, env, names);
    }
    return names;
}
...
function assertListOrPartial(term, env) {
    if (properListItems(term, env) != null || isPartialList(term, env))
        return;
    ...
    return function* ({ solver, goal, env }) {
        ...
        for (const group of groups) {
            ...
            for (let i = 0; i < free.length; i++) {
                if (!unify(free[i], group.witness.args[i], next)) {
                    matches = false;
                    break;
                }
            }
...
function resolveCallable(term, env) {
    ...
    if (resolved.module != null)
        callable.module = resolved.module;
    return callable;
}
...
function* callClosureBuiltin({ solver, goal, env }) {
    ...
    if (closure.module != null)
        invoked.module = closure.module;
    const child = solver.cloneForInnerGoal();
    try {
...
function* callNthBuiltin({ solver, goal, env }) {
    ...
    if (requestedTerm.type !== VAR) {
        ...
        if (requested < 0n)
            throw new PrologError('domain_error(not_less_than_zero)', requestedTerm);
    }
    const child = solver.cloneForInnerGoal();
    let nth = 0n;
    try {
        for (const answerEnv of child.solve([invoked], env, 0)) {
            nth++;
            if (requested != null && nth < requested)
                continue;
...
export function formalErrorTerm(error) {
    ...
    if (error.formalTerm != null)
        return compound('error', [error.formalTerm, context]);
...
function* catchBuiltin({ solver, goal, env }) {
    ...
    try {
        ...
        if (!unify(goal.args[1], ball, recovered))
            throw error;
...
function* negationBuiltin({ solver, goal, env }) {
    for (const _ of solver.cloneForInnerGoal(1).solve([callable(goal.args[0], env)], env.clone(), 0))
        return;
    yield env;
}
...
function* ifThenBuiltin({ solver, goal, env }) {
    for (const conditionEnv of solver.cloneForInnerGoal(1).solve([callable(goal.args[0], env)], env.clone(), 0)) {
        for (const consequentEnv of solver.solve([callable(goal.args[1], conditionEnv)], conditionEnv, 0)) {
            ...
            if (solver.solutionsSeen > 0)
                solver.solutionsSeen--;
            yield consequentEnv;
        }
        return;
    }
...
function evaluate(term, env) {
    ...
    if (term.type === ATOM) {
        if (term.name === 'pi')
            return { integer: false, value: Math.PI };
        if (term.name === 'e')
            return { integer: false, value: Math.E };
    }
    if (term.type !== COMPOUND)
...
function evaluateOperation(term, args) {
    ...
    if (arity === 1 && name === '\\') {
        if (!args[0].integer)
            throw new PrologError('type_error(integer)', numericTerm(args[0]));
    ...
    if (arity === 1 && ['abs', 'sign', 'float', 'truncate', 'round', 'ceiling', 'floor',
        ...
        if (name === 'abs' && args[0].integer)
            return { integer: true, value: args[0].value < 0n ? -args[0].value : args[0].value };
        if (name === 'sign' && args[0].integer)
            return { integer: true, value: args[0].value < 0n ? -1n : args[0].value > 0n ? 1n : 0n };
    ...
    if (bothInteger && ['+', '-', '*', '//', 'div', 'mod', 'rem', '/\\', '\\/', 'xor', '<<', '>>'].includes(name)) {
        ...
        if (name === 'rem')
            return { integer: true, value: a % b };
        if (name === 'mod')
            return { integer: true, value: ((a % b) + b) % b };
        if (name === '/\\')
            return { integer: true, value: a & b };
        if (name === '\\/')
            return { integer: true, value: a | b };
    ...
    if (!Number.isFinite(value))
        throw new PrologError('evaluation_error(float_overflow)');
    const underflow = value === 0 && ((name === '*' && x !== 0 && y !== 0) ||
        (name === '/' && x !== 0) ||
        ((name === '**' || name === '^') && x !== 0));
    if (underflow)
        throw new PrologError('evaluation_error(underflow)');
...
function* isBuiltin({ goal, env }) {
    ...
    const next = env.clone();
    if (unify(goal.args[0], result, next))
        yield next;
}
...
export function getStrictIsoRegistry() {
    // @ts-expect-error TS7005: auto-suppressed
    if (strictIsoRegistry == null)
        strictIsoRegistry = createStrictIsoRegistry();
    // @ts-expect-error TS7005: auto-suppressed
