...
export async function main(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (!endOptions && arg === '--') {
            ...
            const goal = argv[++i];
            if (goal == null)
                throw new Error(`option ${arg} requires a goal`);
    ...
    for (const file of options.files) {
        if (file === '-') {
            if (usedStdin)
                throw new Error("stdin input '-' can only be used once");
            usedStdin = true;
            ...
            if (!response.ok)
                throw new Error(`could not fetch URL: ${file}`);
    ...
    if (options.goals.length === 0 && !options.quads) {
        ...
        for (const source of sourceParts)
            options.goals.push(...goalsFromSource(source.text));
    }
...
async function loadExplanation() {
    if (explanationModule == null)
        explanationModule = await import('./explain.js');
    ...
    try {
        ...
        for (const goal of goals) {
            ...
            for (const env of solver.solve([goal], new engine.Env(), 0)) {
                if (!engine.termIsGround(goal, env))
                    continue;
                ...
                if (facts.has(line) || lines.has(line))
                    continue;
                lines.add(line);
                process.stdout.write(line);
                if (options.proof)
                    writeExplanation(explanation, program, engine.copyResolved(goal, env), registry);
            }
        }
    }
    catch (error) {
...
function writeExplanation(explanation, program, resolved, registry) {
    ...
    if (!proof.ok)
        process.stdout.write(explanation.whyNoProof(resolved));
}
