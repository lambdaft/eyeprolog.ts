...
ATOM, COMPOUND, VAR, Env, atom, compound, deref, emptyList, flattenConjunction, variable, } from './term.js';
...
function terminalItems(term, env = new Env()) {
    ...
    if (cursor.type === ATOM && cursor.name === '[]')
        return items;
    if (cursor.type === VAR)
...
function appendStateArguments(nonterminal, input, output, module) {
    ...
    if (nonterminal.type !== COMPOUND)
        throw new PrologError('type_error(callable)', nonterminal);
    if (nonterminal.name === ':' && nonterminal.arity === 2) {
        const qualifier = nonterminal.args[0];
        if (qualifier.type === VAR)
            throw new PrologError('instantiation_error');
        if (qualifier.type !== ATOM)
            throw new PrologError('type_error(atom)', qualifier);
...
function markGoalModule(term, module) {
    ...
    if (term.type === COMPOUND &&
        ...
        for (const argument of term.args)
            markGoalModule(argument, module);
    }
    return term;
}
...
export function expandDcgBody(body, input, output, options = {}) {
    ...
    if (body.type === ATOM && body.name === '{}')
        return equality(input, output);
    if (isTerminalSequence(body))
        return terminalsGoal(body, input, output, env);
    if (body.type === COMPOUND && body.name === ',' && body.arity === 2) {
        const middle = freshDcgVariable('sequence');
        return conjunction(expandDcgBody(body.args[0], input, middle, options), expandDcgBody(body.args[1], middle, output, options));
    }
    ...
    if (body.type === COMPOUND && body.name === '\\+' && body.arity === 1) {
        ...
        const expandedNegated = negated.type === ATOM || negated.type === COMPOUND || negated.type === VAR
            ...
            : negated;
        return conjunction(compound('\\+', [expandedNegated]), equality(input, output));
    }
    if (body.type === COMPOUND && body.name === '->' && body.arity === 2) {
        const middle = freshDcgVariable('condition');
        return compound('->', [
...
export function validateDcgEmbeddedGoals(body, input, output) {
    ...
    if (invalidBody != null)
        throw new PrologError('type_error(callable)', invalidBody);
    const visit = (term) => {
        if (term.type !== COMPOUND)
            return;
...
function invalidDcgControl(term) {
    if (term.type !== ATOM && term.type !== COMPOUND && term.type !== VAR)
        return term;
    if (term.type !== COMPOUND || ![',', ';', '|', '->'].includes(term.name))
        return null;
    for (const argument of term.args) {
        if (argument.type === COMPOUND && argument.name === '{}' && argument.arity === 1)
            continue;
...
function splitGrammarHead(head) {
    ...
    let module = null;
    if (head.type === COMPOUND && head.name === ':' && head.arity === 2) {
        if (head.args[0].type === VAR)
            throw new PrologError('instantiation_error');
        if (head.args[0].type !== ATOM)
            throw new PrologError('type_error(atom)', head.args[0]);
...
export function expandDcgRuleClause(clause, defaultModule = 'user') {
    if (clause?.body?.length !== 0 || clause?.head?.type !== COMPOUND ||
        clause.head.name !== '-->' || clause.head.arity !== 2)
        return null;
    ...
    if (declaration.terminals != null)
        bodyOutput = freshDcgVariable('semicontext');
    let expandedBody = expandDcgBody(clause.head.args[1], input, bodyOutput, { module });
    if (declaration.terminals != null) {
        expandedBody = conjunction(expandedBody, terminalsGoal(declaration.terminals, output, bodyOutput, new Env()));
    }
    const expanded = {
        head: expandedHead,
